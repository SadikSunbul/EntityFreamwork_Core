...
var datas = await context.Persons.TemporalAsOf(new DateTime(2022, 12, 09, 05, 30, 04)).Select(p => new
{
    p.Id,
    p.Name,
    PeriodStart = EF.Property<DateTime>(p, "PeriodStart"), //bunlar sadov property olarak tutuluyor EF NIN PROPERTYSINDEN DATETIME TURUNDEN Adı PeriodStart bu olanın degerını getırır
...
var dateOfDelete = await context.Persons.TemporalAll()
    .Where(p => p.Id == 3)
    .OrderByDescending(p => EF.Property<DateTime>(p, "PeriodEnd"))  //shadow property oldugu ıcın boyle yazdık ona gore bır sıralama yap dedık 
    .Select(p => EF.Property<DateTime>(p, "PeriodEnd")) //bunu sec dedık 
    .FirstAsync(); //ilkini ver dedik

var deletedPerson = await context.Persons.TemporalAsOf(dateOfDelete.AddMilliseconds(-1)) //1ms gerı ye aldık farklı bır calısması var ondan 
    .FirstOrDefaultAsync(p => p.Id == 3);

await context.AddAsync(deletedPerson);

await context.Database.OpenConnectionAsync(); //bunu yazmayıncada hata verıcektır --> baglantıyı actık

await context.Database.ExecuteSqlInterpolatedAsync($"SET IDENTITY_INSERT dbo.Persons ON"); //bunu acmazsak hata verır cunkı ıd degerını kendısı atıyordu yukarıda gelen verıde bız kendımız ıd degerını vermıs gıbı olduk 
await context.SaveChangesAsync();
await context.Database.ExecuteSqlInterpolatedAsync($"SET IDENTITY_INSERT dbo.Persons OFF"); //işimiz bittikten sonra kapıyoruz
