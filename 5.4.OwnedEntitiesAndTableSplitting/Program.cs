...
//cogul oldugu ıcın data basede bunun yerıne bır tablo olusturulacaktır tek fark dbset property olarak yazmadık erısım saglanamaz orders lara
...
//Herhangı bır owned entıty typ ıcın dbset propertysıne ıhtıyac yoktur 
//onModelCreating fonksıyonunda entitiy<t> metodu ile owned entity type yutunden bır sınıf uzerınden herhangı bır konfıgrasyon gerceklestırılemez!
//owned entıty typlerın kalıtımsal hıyerasisi olmaz 
#endregion
...
//[Owned]  //burayı yazmassak efcore anlamaz
class EmployeeName
{
    public string Name { get; set; }
    public string MiddleName { get; set; }
...
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            builder.WithOwner().HasForeignKey("OwnedEmployeeId"); //forenkey olusturduk
            builder.Property<int>("Id");
            builder.HasKey("Id"); //pk dedik
        });

        #endregion

        modelBuilder.ApplyConfiguration(new EmployeeConfiguration()); //assambliy gibi
    }
