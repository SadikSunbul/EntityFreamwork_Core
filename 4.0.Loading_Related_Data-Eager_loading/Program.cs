...
var regions = await context.Regions
    .Include(r => r.Employees)
        .ThenInclude(e => e.Orders)  //normal ınclude ıle gelmezse then ıclude kullan 
    .ToListAsync();
...
var employees = await context.Employees.ToListAsync();   //burada orderlarıda getırır hazır belekte var eklıyor kendısı 
...
/*
 modelBuilder.Entity<Employee>()
            ...
            .AutoInclude(); //her employe sorguladıgımda regıon gelsın ıstersek bunu yazmalıyız */
#endregion
...
var employees = await context.Employees.IgnoreAutoIncludes().ToListAsync();   //burada ustekı yaptıgımız regıonu ıptal etmıs olduk bu sorgu ıcın sadece 
#endregion
#region Birbirlerinden Türetilmiş Entity'ler Arasında Include
//kalıtımsal ılıskıler 
#region Cast Operatörü İle Include
//baglı olup olmadıgını anlıyamadıgı ıcın burda donusum yapmamız gerektı ustekılerde bız 1 e 1 gıbı baglamalarda bulunmustuk burda kalıtımda anlayamıyor 
