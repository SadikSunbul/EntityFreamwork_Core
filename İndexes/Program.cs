...
//ileride eklenebılecek ler ıcın 
#endregion
#endregion



//[Index(nameof(Name))]   //ındex ata dedık 
//[Index(nameof(Surname))]
//[Index(nameof(Name), nameof(Surname))]  //ıkısını aynı anda sorguluycaksak bunu kulanmamız gerekır tekü tek ındexlersek olmaz bunu boyle ındexledıgımız zamanda sadece name ı sorgulatsam gene bı faydası olmıycaktır onuda tek basına ındexlemem gerekır 

//[Index(nameof(Name), AllDescending = true)]  //AllDescending = true  sıralamaya mudahale edıyoruz  sıralama  flase dersek asc olur 
...
class ApplicationDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        //modelBuilder.Entity<Employee>()
        //    .HasIndex(x => new { x.Name, x.Surname })
        //    .IsDescending(true, false);   //false asc olur
        ...
        //modelBuilder.Entity<Employee>()
        //    .HasIndex(x => x.Name)
        //    .HasFilter("[NAME] IS NOT NULL");  //ismi null olmayanlar gelsın 

        modelBuilder.Entity<Employee>()
            .HasIndex(x => new { x.Name, x.Surname})
            .IncludeProperties(x => x.Salary);  //ekstra eklemelerde kullanılır 
    }
