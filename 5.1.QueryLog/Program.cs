...
class ApplicationDbContext : DbContext
{
    ...
    //readonly ILoggerFactory loggerFactory = LoggerFactory.Create(builder => builder.AddConsole());  consolo log lanıcaktır sorguyu 
    //filtreleme altaki gibi yapılıyor 
    readonly ILoggerFactory loggerFactory = LoggerFactory.Create(builder => builder
    .AddFilter( (category, level) =>
    {
        return category == DbLoggerCategory.Database.Command.Name && level == LogLevel.Information; //buraya esıtse loglama yapılsın dedık 
    })
    .AddConsole());
